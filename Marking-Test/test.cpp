...
TEST(StudentHolder, Constructor_From_Filepath_Small_Students_JSON) {
	...
	Student student_0 = s.getStudent("JS1234568");
	Student student_1 = s.getStudent("GF1234568");
	...
	EXPECT_EQ(student_0.getIdentifier(), "JS1234568");
	...
	EXPECT_EQ(student_1.getIdentifier(), "GF1234568");
...
TEST(CourseHolder, Constructor_From_Filepath_Small_Courses_JSON) {
	...
	std::vector<double> AMF_weights = { 20, 20, 10, 50 };
	EXPECT_EQ((*s.getCourse("AMF123")).getWeights(), AMF_weights);
	//EXPECT_EQ(typeid((*s.getCourse("AMF123"))).name(), "ExamOnly");

	std::vector<double> NLA_weights = { 1, 2, 3, 94 };
	EXPECT_EQ((*s.getCourse("NLA123")).getWeights(), NLA_weights);
	//EXPECT_EQ(typeid((*s.getCourse("NLA123"))).name(), "CourseworkOnly");

	std::vector<double> PDE_weights = { 1.0, 3.3, 10, 85.7 };
